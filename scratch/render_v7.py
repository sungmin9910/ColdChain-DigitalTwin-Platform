...
ax1.add_patch(patches.Rectangle((130.0, 127.5), 13.9, 18.5, facecolor='#336699', edgecolor='yellow', lw=1.5, alpha=0.75))
ax1.text(136.95, 136.0, 'BH1750 (GY-302)\n13.9 x 18.5mm', color='white', ha='center', va='center', fontsize=8, fontweight='bold')
for x in [131.87 + i*2.54 for i in range(5)]:
    ax1.add_patch(patches.Circle((x, 144.5), 0.8, facecolor=PAD_COLOR, edgecolor='black', lw=0.5))
ax1.add_patch(patches.Rectangle((131.87 - 1.27, 144.5 - 1.27), 10.16 + 2.54, 2.54, facecolor='none', edgecolor='#ff4757', lw=1.2, ls='--'))

# 3. JST-PH and SW1 (Top, Center)
ax1.add_patch(patches.Rectangle((120.0, 144.5), 6.0, 4.5, facecolor='#ffffff', edgecolor='black', lw=1))
ax1.text(123.0, 146.75, 'JST-PH', color='black', ha='center', va='center', fontsize=6)
ax1.add_patch(patches.Rectangle((119.5, 137.0), 7.0, 4.0, facecolor='#444444', edgecolor='white', lw=1))
ax1.text(123.0, 139.0, 'SW1', color='white', ha='center', va='center', fontsize=6)

# --- BOTTOM SIDE ---
# 1. SHT45 (Bottom, Top-Left - Horizontal: 25.5W x 17.8H)
# X in [100.5, 126.0], Y in [125.5, 143.3] -> COMPLETELY CLEARS M3 HOLE H3 (Y < 143.75)
...
# SHT45 Female Socket (1x05 horizontal)
...
# 2. GY-521 (Bottom, Top-Right - Horizontal: 20.7W x 15.65H)
# X in [127.0, 147.7], Y in [126.5, 142.15] -> COMPLETELY CLEARS SHT45 (X >= 127.0 > 126.0) and M3 HOLE H4 (Y < 143.75)
ax2.add_patch(patches.Rectangle((127.0, 126.5), 20.7, 15.65, facecolor='#d63031', edgecolor='orange', lw=1.5, alpha=0.75))
ax2.text(137.35, 136.0, 'GY-521 (MPU6050)\n20.7 x 15.65mm', color='white', ha='center', va='center', fontsize=8, fontweight='bold')
# GY-521 Female Socket (1x08 horizontal)
for x in [128.46 + i*2.54 for i in range(8)]:
    ax2.add_patch(patches.Circle((x, 128.0), 0.8, facecolor=PAD_COLOR, edgecolor='black', lw=0.5))
ax2.add_patch(patches.Rectangle((128.46 - 1.27, 128.0 - 1.27), 17.78 + 2.54, 2.54, facecolor='none', edgecolor='#ff4757', lw=1.2, ls='--'))

# 3. ATGM336H GPS (Bottom, Right-Wing) - 13.1W x 15.7H
# X in [136.5, 149.6], Y in [108.0, 123.7]
...
plt.tight_layout()
out_png = 'scratch/preview_clean_layout_v7.png'
plt.savefig(out_png, dpi=150)
print(f"Generated v7 preview: {out_png}")

...
# Build the new header with updated footprint pad nets
# 1. Base kicad_pcb setup
...
# -------------------------------------------------------------
# J1_1 (122.0, 146.5) -> SW1_1 (114.5, 146.5)
...
# -------------------------------------------------------------
# SW1_2 (116.0, 146.5) -> J_MCU_R_1 (133.89, 102.66)
...
add_track(116.11, 105.20, 102.33, 105.20, "F.Cu", 2)
add_track(102.33, 105.20, 102.33, 109.50, "F.Cu", 2) # into GPS Pin 1!

# Feed 2: South to SHT45 Pin 4 (115.79, 127.50) & Pin 5 (118.33, 127.50)
# Bypass MCU_L along western edge X=114.50
...
# Feed 3: South to R3 Pin 2 (122.75, 142.00)
...
# Feed 4: Across BAT spine via Via bridge to R4 Pin 2, MPU6050 Pin 8, and BH1750 Pin 5
# Bridge from (124.0, 143.5) to (126.0, 143.5) across X=125 BAT spine
add_track(122.75, 143.50, 124.00, 143.50, "F.Cu", 2)
add_via(124.00, 143.50, 2)
add_track(124.00, 143.50, 126.00, 143.50, "B.Cu", 2)
add_via(126.00, 143.50, 2)
add_track(126.00, 143.50, 128.75, 143.50, "F.Cu", 2)
add_track(128.75, 143.50, 128.75, 142.00, "F.Cu", 2) # into R4 Pin 2!

# Continue East on F.Cu to East edge X=147.50
add_track(128.75, 143.50, 147.31, 143.50, "F.Cu", 2)
add_track(147.31, 143.50, 147.31, 127.50, "F.Cu", 2) # into MPU6050 Pin 8!
add_track(147.31, 127.50, 147.83, 127.50, "F.Cu", 2)
add_track(147.83, 127.50, 147.83, 123.20, "F.Cu", 2) # into BH1750 Pin 5!

# -------------------------------------------------------------
# 4. NET 5: I2C_SDA (Bottom B.Cu)
# -------------------------------------------------------------
# Source: J_MCU_R Pin 6 (133.89, 115.36)
# Target 1: MPU6050 Pin 5 (139.69, 127.50) & BH1750 Pin 2 (140.21, 123.20)
add_track(133.89, 115.36, 139.69, 115.36, "B.Cu", 5)
add_track(139.69, 115.36, 139.69, 127.50, "B.Cu", 5) # into MPU6050 Pin 5!
add_track(139.69, 120.00, 140.21, 120.00, "B.Cu", 5)
add_track(140.21, 120.00, 140.21, 123.20, "B.Cu", 5) # into BH1750 Pin 2!

# Target 2: West along Highway at Y=125.00 to SHT45 Pin 1 (108.17, 127.50) & R3 Pin 1
add_track(133.89, 115.36, 131.00, 115.36, "B.Cu", 5)
add_track(131.00, 115.36, 131.00, 125.00, "B.Cu", 5)
add_track(131.00, 125.00, 108.17, 125.00, "B.Cu", 5) # Highway at Y=125.00!
...
# Target 1: MPU6050 Pin 6 (142.23, 127.50) & BH1750 Pin 3 (142.75, 123.20)
add_track(133.89, 117.90, 142.23, 117.90, "B.Cu", 6)
add_track(142.23, 117.90, 142.23, 127.50, "B.Cu", 6) # into MPU6050 Pin 6!
add_track(142.23, 120.00, 142.75, 120.00, "B.Cu", 6)
add_track(142.75, 120.00, 142.75, 123.20, "B.Cu", 6) # into BH1750 Pin 3!

# Target 2: West along Highway at Y=130.00 to SHT45 Pin 2 (110.71, 127.50) & R4 Pin 1
add_track(133.89, 117.90, 134.64, 117.90, "B.Cu", 6)
add_track(134.64, 117.90, 134.64, 130.00, "B.Cu", 6)
add_track(134.64, 130.00, 110.71, 130.00, "B.Cu", 6) # Highway at Y=130.00!
...
# -------------------------------------------------------------
# From J_MCU_R Pin 4 (133.89, 110.28) to J_GPS Pin 4 (109.95, 109.50)
# Run around MCU_L via South channel at Y=121.50
add_track(133.89, 110.28, 136.00, 110.28, "B.Cu", 7)
add_track(136.00, 110.28, 136.00, 121.50, "B.Cu", 7)
add_track(136.00, 121.50, 109.95, 121.50, "B.Cu", 7) # cross from X=136 to X=109.95 below MCU_L
add_track(109.95, 121.50, 109.95, 109.50, "B.Cu", 7) # straight North into GPS Pin 4!

# -------------------------------------------------------------
# 7. NET 8: GPS_TX (Bottom B.Cu)
# -------------------------------------------------------------
# From J_MCU_R Pin 5 (133.89, 112.82) to J_GPS Pin 3 (107.41, 109.50)
# Run around MCU_L via South channel at Y=123.00
add_track(133.89, 112.82, 137.50, 112.82, "B.Cu", 8)
add_track(137.50, 112.82, 137.50, 123.00, "B.Cu", 8)
add_track(137.50, 123.00, 107.41, 123.00, "B.Cu", 8) # cross below MCU_L at Y=123.00
add_track(107.41, 123.00, 107.41, 109.50, "B.Cu", 8) # straight North into GPS Pin 3!

# -------------------------------------------------------------
# 8. NET 1: GND (Complete Distribution on Top and Bottom)
# -------------------------------------------------------------
...
# 2) SHT45 Pin 3 (113.25, 127.50) -> H3 (103.5, 146.5)
...
# J1 Pin 2 (124.0, 146.5) into H4 ground
...
# 5) H2 (146.5, 103.5) -> H4 (146.5, 146.5) along east edge at X=148.50
...
add_track(145.29, 123.20, 145.29, 117.50, "F.Cu", 1)
add_track(145.29, 117.50, 148.50, 117.50, "F.Cu", 1) # taps to X=148.5 GND line!
add_track(137.67, 123.20, 137.67, 117.50, "F.Cu", 1)
add_track(137.67, 117.50, 145.29, 117.50, "F.Cu", 1) # joins BH1750 Pin 1 to Pin 4 GND!
...
add_track(132.07, 137.00, 146.50, 137.00, "F.Cu", 1)
add_track(146.50, 137.00, 146.50, 146.50, "F.Cu", 1) # into H4!

# Tie H1 to H2 along top edge
add_track(103.50, 103.50, 105.50, 101.50, "B.Cu", 1)
add_track(105.50, 101.50, 116.11, 101.50, "B.Cu", 1)
add_track(116.11, 101.50, 133.89, 101.50, "B.Cu", 1)
add_track(133.89, 101.50, 144.50, 101.50, "B.Cu", 1)
add_track(144.50, 101.50, 146.50, 103.50, "B.Cu", 1) # Top GND bridge!

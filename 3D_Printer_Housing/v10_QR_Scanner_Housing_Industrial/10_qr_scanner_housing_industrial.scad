/* 
 * Ergonomic Pistol Grip GM77/ESP32 QR Scanner Housing Design V10.0 (Simplified & Print-Friendly)
 * 
 * Features:
 *   - 3-Piece Assembly: Left Half, Right Half, and Trigger Button.
 *   - Integrated Front Bumper Bezel (no separate bumper print required).
 *   - Simplified FDM-friendly mounting:
 *     - ESP32 DevKitC V4: Rests on 4 flat corner shelves (14.5mm clearance below for wiring). Secured by clamping ribs on the Right Half.
 *     - GM77: Simple open pocket with thick walls and cut-out back wall.
 *     - SSD1306 OLED: Rests on simple support ledges behind the front window.
 *     - 18650 Battery: Lies in a semi-cylindrical trough (cradle) inside the handle, clamped by matching ribs.
 *     - TP4056 & Voltly Modules: Slide vertically into thick-walled, open U-pockets on the inner wall of the Left Half.
 *     - Micro Tact Switch (AK-TS-I015-42): Slides into a simple rectangular pocket behind the trigger plunger.
 *     - MSL-1C2P Slide Switch: Retains the recessed panel-mount slot on the handle side.
...
module housing_left_half() {
    difference() {
        union() {
            ...
            // A. OLED Flat Ceiling Guide (Left Half side)
            translate([head_w/2, 60.0, head_h - wall]) {
                // Left guide wall & shelf (grips left edge of oled PCB)
                translate([-oled_size[0]/2 - clearance - 2.0, -oled_size[1]/2, -5.0]) {
                    difference() {
                        cube([6.0, oled_size[1], 5.0]); // solid block
                        translate([2.0, -0.1, 1.5])
                        cube([4.2, oled_size[1] + 0.2, 3.6]); // slot for PCB edge (3.5mm thick)
                    }
                }
            }
            
            // B. GM77 Pocket Tray (Left Half side)
            ...
            // C. ESP32 Left Corner Support Ledges (Z = wall + 14.0, giving 14mm clearance)
            translate([head_w/2, head_l - 32.0, 0]) {
                let(x_edge = -esp32_size[0]/2, y_front = -27.2, y_rear = 27.2) {
                    // Front-Left Ledge
                    translate([x_edge - 1.5, y_front - 1.0, wall])
                    cube([4.0, 6.0, 14.0]);
                    // Rear-Left Ledge
                    translate([x_edge - 1.5, y_rear - 5.0, wall])
                    cube([4.0, 6.0, 14.0]);
                }
            }
...
module housing_right_half() {
    difference() {
        union() {
            ...
            // A. OLED Flat Ceiling Guide (Right Half side)
            translate([head_w/2, 60.0, head_h - wall]) {
                // Right guide wall & shelf (grips right edge of oled PCB)
                translate([oled_size[0]/2 + clearance - 4.0, -oled_size[1]/2, -5.0]) {
                    difference() {
                        cube([6.0, oled_size[1], 5.0]); // solid block
                        translate([-0.2, -0.1, 1.5])
                        cube([4.2, oled_size[1] + 0.2, 3.6]); // slot for PCB edge
                    }
                }
            }
            
            // B. GM77 Pocket Tray (Right Half side)
            translate([head_w/2, 27.0, wall + 6.75]) {
                difference() {
            ...
            // C. ESP32 Right Corner Support Ledges & Upper Clamping Ribs
            translate([head_w/2, head_l - 32.0, 0]) {
                let(x_edge = esp32_size[0]/2, y_front = -27.2, y_rear = 27.2) {
                    // Front-Right Ledge
                    translate([x_edge - 2.5, y_front - 1.0, wall])
                    cube([4.0, 6.0, 14.0]);
                    // Rear-Right Ledge
                    translate([x_edge - 2.5, y_rear - 5.0, wall])
                    cube([4.0, 6.0, 14.0]);
                }
                
                // D. ESP32 Clamping Rib (presses down on top of ESP32 PCB when closed)
                // ESP32 PCB top is at Z = wall + 14.0 + 1.6 = 18.1.
                // Clamping rib extends from top ceiling (Z = head_h - wall = 47.5) down to Z = 18.2
                // Placed near the middle-back of the PCB to avoid components
                translate([esp32_size[0]/2 - 2.0, -10.0, 18.2])
                cube([2.0, 20.0, head_h - wall - 18.2 - 2.0]);
            }
...
module full_housing_cuts() {
    // Front optical window cut (for GM77 camera field of view)
    translate([head_w/2 - 13.0, -5.0, 0.0])
    cube([26.0, 14.0, 16.0]);

    // OLED Window Cutout (on the top ceiling of the head)
    translate([head_w/2, 60.0 + 4.57, head_h - 4.0])
    rounded_rect(23.0, 12.0, 10.0, 1.5, true);

    // Rear USB slot for programming ESP32
    translate([head_w/2 - 6.5, head_l - 10.0, wall + 14.0])
    cube([13.0, 15.0, 8.5]);
    ...
    // GM77 Screw Pilot holes (M2, 1.8mm tap)
...
module tp4056_u_pocket(is_left) {
    // Vertical U-pocket at handle base: open on the split plane side (+X)
    // Board is 25mm (Z) x 19.0mm (Y). Pocket walls are 2.5mm thick.
    z_bottom = -handle_h + 3.0; // Z = -102
    pcb_w = tp4056_size[1] + 2*clearance; // 19.7
    pcb_t = tp4056_size[2] + 0.2; // 3.7
...
module tp4056_clamping_rib(is_right) {
    // A simple flat rib on the Right Half that presses against the flat back of the TP4056 PCB
    z_bottom = -handle_h + 3.0;
    pcb_w = tp4056_size[1] + 2*clearance;
    pcb_t = tp4056_size[2] + 0.2;
    
    // Sits at X = 0.5 to press PCB into left pocket
    translate([0.6, 0, z_bottom + 12.5])
    cube([1.5, pcb_w - 4.0, 20.0], center=true);
}

module stepup_u_pocket(is_left) {
    // Similar U-pocket for Voltly boost module (22mm Z x 11mm Y)
    z_bottom = -handle_h + 32.0; // Z = -73
    pcb_w = stepup_size[1] + 2*clearance; // 11.7
    pcb_t = stepup_size[2] + 0.2; // 3.8
...
module tact_switch_pocket(is_left) {
    // Switch sits at Y = -handle_top_d/2 + 7.0, Z = -20
    // Body is 6x6x5mm. Plunger button faces -Y.
...
module housing_shape(wall_thick=0) {
    ...
    // The front 6mm (from Y=2 to Y=8) represents the integrated bumper ring.
    // In assembly visualization, it's rendered as part of the body.
    hull() {
...
module trigger_button() {
    // Standard cylinder body that slides smoothly in handle front hole.
    // Inner stop flange prevents the button from falling out.
    // Plunger extension on back triggers the micro tact switch button.
    rotate([90, 0, 0]) {
...
module esp32_mockup() {
    ...
    // Visual down-ward pin headers (for verification)
    color("Black") {
...
module all_head_electronics_assembly(is_exploded=false) {
    ...
    // ESP32 (Ledges keep it elevated at Z = wall + 14.0)
    translate([head_w/2, head_l - 32.0, wall + 14.8 + esp32_offset])
    esp32_mockup();
    
    // OLED (Flat against the ceiling at Y=60.0, Z=head_h-wall-0.6)
    translate([head_w/2, 60.0, head_h - wall - 0.6 + oled_offset])
    rotate([0, 0, 0])
    oled_mockup();
}

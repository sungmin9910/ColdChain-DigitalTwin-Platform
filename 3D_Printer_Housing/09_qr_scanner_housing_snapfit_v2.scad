...
esp32_size = [26.8, 51.5, 15.0]; // ESP32 board (reduced width to 26.8 to fit snugly based on user request)
oled_size = [26.0, 27.0, 1.2];   // SSD1306 OLED PCB size
oled_z_mount = head_h - 4.5;     // OLED mount Z coordinate (lowered from head_h - 1.2 to create proper spacing under ceiling)
...
if (render_part == "print_all") {
    ...
    // 2 Plugs for the handle screw holes
    translate([head_w * 2 + 40, -10, 0]) print_plug();
    translate([head_w * 2 + 50, -10, 0]) print_plug();
}

if (render_part == "left_half") print_left();
if (render_part == "right_half") print_right();
if (render_part == "trigger") print_trigger();
if (render_part == "plug") print_plug();
...
module full_housing_solid() {
    union() {
        ...
        // [C] OLED Side-Wall Cantilever brackets (Screwless, supported by side walls, trimmed to outer shell shape)
        intersection() {
            translate([head_w/2, 55, oled_z_mount])
            rotate([0, 0, 0]) {
                // Left Bracket (attached to left side wall)
                difference() {
                    // Solid bracket block extending from far left to X = -13.35, and height to Z_local = 8.0
                    translate([-head_w, -13.5 - clearance, -5.0])
                    cube([head_w - 13.0 - clearance, 27.0 + 2*clearance, 13.0]);
                    
                    // Slot for OLED PCB: 1.5mm deep, 1.2mm high (Z_local: -1.0 to 0.2)
                    translate([-13.0 - clearance - 1.5, -13.5 - clearance - 0.1, -1.0])
                    cube([1.6, 27.0 + 2*clearance + 0.2, 1.2]);
                }
                
                // Right Bracket (attached to right side wall)
                difference() {
                    // Solid bracket block extending from X = 13.35 to far right, and height to Z_local = 8.0
                    translate([13.0 + clearance, -13.5 - clearance, -5.0])
                    cube([head_w, 27.0 + 2*clearance, 13.0]);
                    
                    // Slot for OLED PCB: 1.5mm deep, 1.2mm high (Z_local: -1.0 to 0.2)
                    translate([13.0 + clearance, -13.5 - clearance - 0.1, -1.0])
                    cube([1.6, 27.0 + 2*clearance + 0.2, 1.2]);
                }
            }
            housing_shape(wall_thick=0);
        ...
        // [E] GM77 Pocket Holder & Screw Bosses (REMOVED: Now using screwless gm77_clamping_pillars)
    }
}
...
module full_housing_cuts() {
    ...
    // [D] OLED Flat Outer Bezel Cut (REMOVED: To prevent shell piercing, keeping the curved outer dome intact)

    // [D.2] OLED Window cutout (aligned to 25.3x17.5 glass area, height matched to oled_z_mount)
    translate([head_w/2, 55, oled_z_mount + 1.5])
    rotate([0, 0, 0])
    translate([0, 4.57, 0])
    rounded_rect(25.3, 17.5, 50, 1.5, true);
    ...
    // [G] GM77 Screw holes (REMOVED: Now using screwless groove/slot clamping pillars)
    ...
    rotate([handle_angle, 0, 0]) {
        for (pt = local_points) {
            ...
            if (is_left) {
                ...
                // Countersunk entry (cone from 6.0mm to 3.2mm over 3.0mm depth)
                translate([-r_val - 1.0, y, z_val])
                rotate([0, 90, 0])
                cylinder(h=3.0, d1=6.0, d2=3.2);
                
                // Clearance hole for screw body (diameter 3.2mm)
                translate([-r_val - 1.0 + 3.0, y, z_val])
                rotate([0, 90, 0])
                cylinder(h=r_val - 7.0, d=3.2); // leaves a 5.0mm thick wall
            } else {
                // Pilot hole
...
module esp32_clamping_pillars(is_left) {
    ...
    x_pcb_half = esp32_size[0] / 2; // 13.75
    ...
    for (y_pos = y_corners) {
        if (is_left) {
            // Left pillar placed entirely outside the board, slot open to the inside
            translate([head_w/2 - x_pcb_half - 3.0, y_pos - pillar_l/2, wall]) {
                difference() {
                    cube([3.0, pillar_l, pillar_h]);
                    // Slot cuts 1.5mm deep into the 3mm pillar
                    translate([1.5, -0.1, slot_z]) cube([1.6, pillar_l + 0.2, slot_h]);
                }
            }
        } else {
            // Right pillar placed entirely outside the board, slot open to the inside
            translate([head_w/2 + x_pcb_half, y_pos - pillar_l/2, wall]) {
                difference() {
                    cube([3.0, pillar_l, pillar_h]);
                    // Slot cuts 1.5mm deep into the 3mm pillar
                    translate([-0.1, -0.1, slot_z]) cube([1.6, pillar_l + 0.2, slot_h]);
                }
            }
        }
    }
...
module gm77_clamping_pillars(is_left) {
    x_pcb_half = gm77_size[0] / 2; // 13.75
    pillar_l = 8.0;
    pillar_h = 16.0;   // The groove is at Z = wall + 10.5 to wall + 12.5.
    slot_z = 10.5;
    slot_h = 2.0;
    
    // Y positions correspond exactly to GM77 mounting ears at Y = 14.4 and Y = 40.8
    y_offsets = [14.4, 40.8]; 
    for (y_pos = y_offsets) {
        if (is_left) {
            // Left pillar placed entirely outside the GM77 module, slot open to the inside
            translate([head_w/2 - x_pcb_half - 3.0, y_pos - pillar_l/2, wall]) {
                difference() {
                    cube([3.0, pillar_l, pillar_h]);
                    // Slot cuts 1.5mm deep into the 3mm pillar
            ...
            // Right pillar placed entirely outside the GM77 module, slot open to the inside
            translate([head_w/2 + x_pcb_half, y_pos - pillar_l/2, wall]) {
                difference() {
                    cube([3.0, pillar_l, pillar_h]);
                    // Slot cuts 1.5mm deep into the 3mm pillar
...
module housing_left_half() {
    difference() {
        union() {
            ...
            // Add screwless clamping slots for GM77 on the left side
            gm77_clamping_pillars(true);
        }
...
module housing_right_half() {
    difference() {
        union() {
            ...
            // Add screwless clamping slots for GM77 on the right side
            gm77_clamping_pillars(false);
        }
...
module all_head_electronics_assembly(is_exploded=false) {
    ...
    translate([head_w/2, 55, oled_offset + oled_z_mount])
    rotate([0, 0, 0])
    translate([0, 0, -0.4])
...
module print_plug() {
    translate([0, 0, 0]) screw_hole_plug();
}

module screw_hole_plug() {
    // A simple plastic cap to plug the handle screw holes and make the grip perfectly smooth.
    // The cap has a sloped (countersunk) top of diameter 6.0mm and thickness 3.0mm,
    // and a small insertion stem of diameter 3.0mm and length 4.0mm to plug into the clearance hole.
    cylinder(h=3.0, d1=6.0, d2=3.2); // matches the countersunk cone of the screw hole
    translate([0, 0, 3.0])
    cylinder(h=4.0, d=3.0); // insertion stem
}

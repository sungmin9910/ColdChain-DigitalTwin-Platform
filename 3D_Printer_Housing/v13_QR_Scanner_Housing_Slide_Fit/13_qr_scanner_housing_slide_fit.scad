/* 
 * Ergonomic Pistol Grip GM77/ESP32 QR Scanner Housing Design V13.0 (ESP32 Slide-Fit & Scanner Window)
 * 
 * Features:
 ...
 *   - Screwless GM77 Mounting with Front Scanner Window:
 ...
 *     - [V13.0 Improved] Replaced the bottom-revealing ??cut with a clean rounded_rect optical scan window 
 *       (24.5mm x 11.5mm) aligned to the lens center at Z = 9.25mm.
 *   - ESP32 Slide-Fit Mounting (Pin-Up):
 *     - Flipped ESP32 DevKitC V4 180 degrees (pins facing UPwards in +Z direction).
 *     - [V13.0 Improved] Slide-in guide channels on Left/Right halves (Z_abs: 5.4 to 7.3mm, slot width 1.9mm).
...
module housing_left_half() {
    difference() {
        union() {
            ...
            // C. [V13.0 Improved] ESP32 Left Slide-in Guide Channel (Left Half)
...
module full_housing_cuts() {
    // [V13.0 Improved] GM77 Optical Window Cut (through the front wall)
    // Centered at X = head_w/2, Y = 8.0 (front wall), Z = wall + 6.75 = 9.25.
    // Creates a clean rounded rectangle hole in the front for the lens and illumination.
    translate([head_w/2, 2.0, wall + 6.75])
    rotate([90, 0, 0])
    rounded_rect(24.5, 11.5, 20.0, 1.5, true);

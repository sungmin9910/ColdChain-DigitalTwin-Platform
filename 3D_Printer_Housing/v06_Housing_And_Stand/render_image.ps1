$scadFile = "$PSScriptRoot\06_housing_3d_design.scad"
...
& $openscad -o "$PSScriptRoot\06_assembly_view.png" --render --autocenter --viewall --imgsize=1024,768 --colorscheme="Metallic" $scadFile
...
& $openscad -o "$PSScriptRoot\06_exploded_view.png" --render --autocenter --viewall --imgsize=1024,768 --colorscheme="Metallic" $scadFile
...
& $openscad -o "$PSScriptRoot\06_cross_section_view.png" --render --camera=367,-133,156,24,56,-20 --projection=o --imgsize=1024,768 --colorscheme="Metallic" $scadFile

# Restore to print_all state
(Get-Content $scadFile) -replace 'render_part = "[a-zA-Z_]+";', 'render_part = "print_all";' | Set-Content $scadFile

# Safe copy of the file via PowerShell to handle Korean encoding issues
Copy-Item -Force $scadFile "$PSScriptRoot\06_하우징_3D_설계_오픈스카드.scad"

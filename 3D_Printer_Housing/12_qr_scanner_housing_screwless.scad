/* 
 * Ergonomic Pistol Grip GM77/ESP32 QR Scanner Housing Design V12.0 (Screwless & ESP32 Pin-Up)
 * 
 * Features:
 ...
 *   - ESP32 Pin-Up Mounting:
 *     - Flipped ESP32 DevKitC V4 180 degrees (pins facing UPwards in +Z direction).
 *     - Board lowered to rest on 3.0mm high corner shelves (Z_pcb_bottom = 5.5mm).
 *     - FDM-clamping rib on Right Half extended down to Z = 7.2mm to secure the board.
 *     - Open headspace above ESP32 allows clean wiring/DuPont connection without collision.
...
module housing_left_half() {
    difference() {
        union() {
            ...
            // C. ESP32 Left Corner Support Ledges (Lowered for Pin-Up: Z = wall + 3.0, giving 3.0mm clearance)
...
module housing_right_half() {
    difference() {
        union() {
            ...
            // [V12.0 Added] GM77 Clamping Rib (Presses down on top of GM77 at Z = 16.5)
            ...
            translate([head_w/2, head_l - 32.0, 0]) {
                let(x_edge = esp32_size[0]/2, y_front = -27.2, y_rear = 27.2) {
                    // Front-Right Ledge (Lowered to 3mm)
                    translate([x_edge - 2.5, y_front - 1.0, wall])
                    cube([4.0, 6.0, 3.0]);
                    // Rear-Right Ledge (Lowered to 3mm)
                    translate([x_edge - 2.5, y_rear - 5.0, wall])
                    cube([4.0, 6.0, 3.0]);
                }
                
                // D. [V12.0 Modified] ESP32 Clamping Rib (presses down on top of ESP32 PCB when closed)
                // ESP32 PCB top is now at Z = wall + 3.0 + 1.6 = 7.1.
                // Clamping rib extends from top ceiling (Z = head_h - wall = 47.5) down to Z = 7.2
                // Placed near the middle-back of the PCB to avoid components
                translate([esp32_size[0]/2 - 2.0, -10.0, 7.2])
                cube([2.0, 20.0, head_h - wall - 7.2 - 2.0]);
            }
...
module full_housing_cuts() {
    ...
    // [V12.0 Modified] Lowered Z coordinate to match the lowered ESP32 position (Z_pcb = 5.5mm)
...
module esp32_mockup() {
    ...
    // [V12.0 Modified] Visual UP-ward pin headers (for verification)
    color("Black") {
